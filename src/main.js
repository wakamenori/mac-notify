...
function assertRootFrameStable() {
    if (!dom.panel || !dom.overlays) {
        return;
    }
    const children = Array.from(root.children);
    const isStable = children.length === 2 &&
...
function renderHeaderActions(view, _loading) {
...
function renderGroups(groups) {
    ...
    if (!state.loading && groups.length === 0) {
        dom.notificationsView.replaceChildren(create("div", "empty", "現在表示できる通知はありません。"));
        return;
    }
...
function renderGroupContent(group, groupIdx) {
...
function renderDialogs(selected, confirm) {
    if (!dom.overlays) {
        return;
    }
    const overlays = [];
...
function renderSettingsView(container) {
    ...
    if (state.prompts.length === 0 && !state.editingPrompt) {
        elements.push(create("div", "empty", "アプリプロンプトはまだ登録されていません。"));
    }
